...
class PlayersTest(BaseCloudkitTest):
    ...
    def test_players_online(self):
        ...
        with patch("driftbase.models.db.utcnow") as mock_date:
            mock_date.return_value = datetime.datetime.now(datetime.UTC) + datetime.timedelta(minutes=5, seconds=1)
            r = self.get(self.endpoints["my_player"])
    ...
    def test_change_name(self):
        ...
        with mock.patch("driftbase.api.players._get_shoutout", return_value=shoutout_mock):
            ...
            shoutout_mock.message.assert_called_once_with("player:name_updated", player_id=r.json().get('player_id'), previous_name=old_name, player_name=r.json().get('player_name'),
    ...
    def test_change_name_from_event(self):
        ...
        with (mock.patch("driftbase.api.players._get_shoutout", return_value=shoutout_mock),
            ...
            shoutout_mock.message.assert_called_once_with("player:name_updated", player_id=player_id,
                                                          previous_name=player_old_name,
